...
author: Nisab Mohd
description: Explore the dynamic world of WeakMaps and WeakSets in JavaScript, unraveling their unique features and applications for efficient memory management and secure data storage.
...
WeakMap does not support iteration and methods <Highlight>keys ( )</Highlight>, <Highlight>values ( )</Highlight>, <Highlight>entries ( )</Highlight>, so there’s no way to get all keys or values from it.

WeakMap has only the following methods:

- <Highlight>weakMap.set(key, value)</Highlight>
- <Highlight>weakMap.get(key)</Highlight>
- <Highlight>weakMap.delete(key)</Highlight>
- <Highlight>weakMap.has(key)</Highlight>
...
WeakSet behaves similarly as WeakMap only difference is no value to a object key.

- It is analogous to Set, but we may only add objects to WeakSet (not primitives).
- Like Set, it supports add, has and delete, but not <Highlight>size</Highlight>, <Highlight>keys ( )</Highlight> and no iterations.
...
Their main advantages are that they have weak reference to objects, so they can easily be removed by garbage collector. That comes at the cost of not having support for clear, size, keys, values…
